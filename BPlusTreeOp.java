...
class BPlusTreeOp extends ApplicationFrame {
    private Connection connection;
    private long totalInsertTime;
    private long totalDeleteTime;
    private long totalQueryTime;
    private Cache<String, Boolean> searchCache; // Cache for search results

    public BPlusTreeOp(Connection connection) {
        super("B+ Tree Performance Metrics");
        this.connection = connection;
        this.searchCache = Caffeine.newBuilder().maximumSize(1000).build(); // Adjust the cache size as needed
    ...
    public boolean search(String username) throws SQLException {
        ...
        Boolean cachedResult = searchCache.getIfPresent(username);
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT COUNT(*) FROM bplus WHERE username = ?")) {
            ...
            // Cache the result
            searchCache.put(username, count > 0);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            BPlusTreeOp bPlusTree = new BPlusTreeOp(connection);
